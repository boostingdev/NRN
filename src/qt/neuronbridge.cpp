...
#include "neuronbridge.moc"

NeuronBridge::NeuronBridge(NeuronGUI *window, QObject *parent) :
    QObject         (parent),
    window          (window),
...
void NeuronBridge::setClientModel()
{
...
// This is just a hook, we won't really be setting the model...
void NeuronBridge::setWalletModel()
{
...
// This is just a hook, we won't really be setting the model...
void NeuronBridge::setMessageModel()
{
    populateMessageTable();
    connectSignals();
}

void NeuronBridge::copy(QString text)
{
    QApplication::clipboard()->setText(text);
}

void NeuronBridge::paste()
{
    emitPaste(QApplication::clipboard()->text());
}

// Options
void NeuronBridge::populateOptions()
{
...
// Transactions
bool NeuronBridge::addRecipient(QString address, QString label, QString narration, qint64 amount, int txnType, int nRingSize)
{
...
void NeuronBridge::clearRecipients()
{
    recipients.clear();
}

bool NeuronBridge::sendCoins(bool fUseCoinControl, QString sChangeAddr)
{
    int inputTypes = -1;
    int nAnonOutputs = 0;
    int ringSizes = -1;
    ...
    {
        int inputType; // 0 NRN, 1 Neuron
        switch(rcp.txnTypeInd)
        {
            case TXT_NRN_TO_NRN:
                formatted.append(tr("<b>%1</b> to %2 (%3)").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::NRN, rcp.amount), Qt::escape(rcp.label), rcp.address));
                inputType = 0;
                break;
    ...
    switch(sendstatus.status)
    {
        ...
        case WalletModel::AmountWithFeeExceedsBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                arg(BitcoinUnits::formatWithUnit(BitcoinUnits::NRN, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        ...
        case WalletModel::SCR_StealthAddressFail:
            QMessageBox::warning(window, tr("Send Coins"),
                ...
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
        case WalletModel::SCR_AmountWithFeeExceedsNeuronBalance:
            QMessageBox::warning(window, tr("Send Coins"),
                tr("The total exceeds your neuron balance when the %1 transaction fee is included.").arg(BitcoinUnits::formatWithUnit(BitcoinUnits::NRN, sendstatus.fee)),
                QMessageBox::Ok, QMessageBox::Ok);
            return false;
...
void NeuronBridge::openCoinControl()
{
...
void NeuronBridge::updateCoinControlAmount(qint64 amount)
{
    CoinControlDialog::payAmounts.clear();
...
void NeuronBridge::updateCoinControlLabels(unsigned int &quantity, int64_t &amount, int64_t &fee, int64_t &afterfee, unsigned int &bytes, QString &priority, QString low, int64_t &change)
{
    emitCoinControlUpdate(quantity, amount, fee, afterfee, bytes, priority, low, change);
}

QVariantMap NeuronBridge::listAnonOutputs()
{
...
void NeuronBridge::populateTransactionTable()
{
...
void NeuronBridge::updateTransactions(QModelIndex topLeft, QModelIndex bottomRight)
{
...
void NeuronBridge::insertTransactions(const QModelIndex & parent, int start, int end)
{
    // New Transactions...
    transactionModel->populateRows(start, end);
}

QString NeuronBridge::transactionDetails(QString txid)
{
...
// Addresses
void NeuronBridge::populateAddressTable()
{
...
void NeuronBridge::updateAddresses(QModelIndex topLeft, QModelIndex bottomRight)
{
    addressModel->poplateRows(topLeft.row(), bottomRight.row());
}

void NeuronBridge::insertAddresses(const QModelIndex & parent, int start, int end)
{
...
QString NeuronBridge::newAddress(QString addressLabel, int addressType, QString address, bool send)
{
...
QString NeuronBridge::lastAddressError()
{
    QString sError;
...
QString NeuronBridge::getAddressLabel(QString address)
{
    return addressModel->atm->labelForAddress(address);
}

void NeuronBridge::updateAddressLabel(QString address, QString label)
{
...
bool NeuronBridge::validateAddress(QString address)
{
    return window->walletModel->validateAddress(address);
}

bool NeuronBridge::deleteAddress(QString address)
{
    return addressModel->atm->removeRow(addressModel->atm->lookupAddress(address));
}

// Messages
void NeuronBridge::appendMessages(QVariantList messages, bool reset)
{
    emitMessages(messages, reset);
}

void NeuronBridge::appendMessage(int row)
{
...
void NeuronBridge::populateMessageTable()
{
...
void NeuronBridge::insertMessages(const QModelIndex & parent, int start, int end)
{
    while(start <= end)
    {
        appendMessage(start++);
...
bool NeuronBridge::deleteMessage(QString key)
{
    return window->messageModel->removeRow(thMessage->mtm->lookupMessage(key));
}

bool NeuronBridge::markMessageAsRead(QString key)
{
    return window->messageModel->markMessageAsRead(key);
}

QString NeuronBridge::getPubKey(QString address)
{
    return addressModel->atm->pubkeyForAddress(address);
}

QString NeuronBridge::addressForPubKey(QString pubkey)
{
    return addressModel->atm->addressForPubkey(pubkey);
}

bool NeuronBridge::setPubKey(QString address, QString pubkey)
{
...
bool NeuronBridge::sendMessage(const QString &address, const QString &message, const QString &from)
{
...
QString NeuronBridge::createGroupChat(QString label)
{
...
QString NeuronBridge::joinGroupChat(QString privkey, QString label)
{
    /*
...
QVariantList NeuronBridge::inviteGroupChat(QString qsaddress, QVariantList invites, QString from)
{
...
void NeuronBridge::connectSignals()
{
...
QString NeuronBridge::translateHtmlString(QString string)
{
    int i = 0;
...
QVariantMap NeuronBridge::userAction(QVariantMap action)
{
...
// Blocks
QVariantMap NeuronBridge::listLatestBlocks()
{
...
QVariantMap NeuronBridge::findBlock(QString searchID)
{
...
QVariantMap NeuronBridge::blockDetails(QString blkHash)
{
...
QVariantMap NeuronBridge::listTransactionsForBlock(QString blkHash)
{
...
QVariantMap NeuronBridge::txnDetails(QString blkHash, QString txnHash)
{
...
QVariantMap NeuronBridge::signMessage(QString address, QString message)
{
    QVariantMap result;
...
QVariantMap NeuronBridge::verifyMessage(QString address, QString message, QString signature)
{
    QVariantMap result;
...
QVariantMap NeuronBridge::getNewMnemonic(QString password, QString language)
{
    QVariantMap result;
...
QVariantMap NeuronBridge::importFromMnemonic(QString inMnemonic, QString inPassword, QString inLabel, bool fBip44, int64_t nCreateTime)
{
...
QVariantMap NeuronBridge::extKeyAccList() {
    QVariantMap result;
...
QVariantMap NeuronBridge::extKeyList() {
    QVariantMap result;
...
QVariantMap NeuronBridge::extKeyImport(QString inKey, QString inLabel, bool fBip44, int64_t nCreateTime)
{
    QVariantMap result;
    ...
    if (eKey58.Set58(sInKey.c_str()) == 0)
    {
        ...
        {
            result.insert("error_msg", "Import failed - Key must begin with Neuroncoin prefix.");
            return result;
        }
...
QVariantMap NeuronBridge::extKeySetDefault(QString extKeyID)
{
    QVariantMap result;
...
QVariantMap NeuronBridge::extKeySetMaster(QString extKeyID)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
    if (extKeyID.length() == 0)
...
QVariantMap NeuronBridge::extKeySetActive(QString extKeyID, QString isActive)
{
    QVariantMap result;
    std::string sInKey = extKeyID.toStdString();
