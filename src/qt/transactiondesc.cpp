...
{
    QString strHTML;
    QString explorer(fTestNet ? "http://testnet.neuronchain.info/" : "http://explorer.neuron.cash/");
    ...
    if (wtx.IsCoinBase() && nCredit == 0)
    {
        ...
        if (wtx.IsInMainChain())
            strHTML += BitcoinUnits::formatWithUnit(BitcoinUnits::NRN, nUnmatured)+ " (" + tr("matures in %n more block(s)", "", wtx.GetBlocksToMaturity()) + ")";
        else
    ...
    if (nNet > 0)
    {
        ...
        // Credit
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::NRN, nNet) + "<br>";
    } else
    {
        ...
        if (fAllFromMe)
        {
            ...
            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
            {
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::NRN, -txout.nValue) + "<br>";
            }
            ...
            if (fAllToMe)
            {
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::NRN, -nValue) + "<br>";
                strHTML += "<b>" + tr("Credit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::NRN, nValue) + "<br>";
            }

            int64_t nTxFee = nDebit - wtx.GetValueOut();
            if (nTxFee > 0)
                strHTML += "<b>" + tr("Transaction fee") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::NRN, -nTxFee) + "<br>";
        } else
        {
            //
            ...
            BOOST_FOREACH(const CTxIn& txin, wtx.vin)
            {
                ...
                if (wallet->IsMine(txin))
                    strHTML += "<b>" + tr("Debit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::NRN, -wallet->GetDebit(txin)) + "<br>";
            };

            BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                if (wallet->IsMine(txout))
                    strHTML += "<b>" + tr("Credit") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::NRN, wallet->GetCredit(txout)) + "<br>";
        }
    }

    strHTML += "<b>" + tr("Net amount") + ":</b> " + BitcoinUnits::formatWithUnit(BitcoinUnits::NRN, nNet, true) + "<br>";
        ...
        BOOST_FOREACH(const CTxIn& txin, wtx.vin)
        {
            ...
            if (txdb.ReadDiskTx(prevout.hash, prev))
            {
                if (prevout.n < prev.vout.size())
                {
                    ...
                    strHTML = strHTML + " " + tr("Amount") + "=" + BitcoinUnits::formatWithUnit(BitcoinUnits::NRN, vout.nValue);
